...
class LTLfTransformer(Transformer):
    ...
    def ltlf_equivalence(self, args):
        ...
        if len(args) == 1:
            ...
            tmp_l = args[l_args-1]
            if(l_args>3):
                subformulas1 = [LTLfNot(args[l_args-3]), args[l_args-1]]
                subformulas2 = [args[l_args-3], LTLfNot(args[l_args-1])]
                tmp_l =   LTLfAnd([LTLfOr(subformulas1), LTLfOr(subformulas2)])
                l_args -= 2
            while(l_args >3):
                subformulas1 = [LTLfNot(args[l_args-3]), tmp_l]
                subformulas2 = [args[l_args-3], LTLfNot(tmp_l)]
                tmp_l =   LTLfAnd([LTLfOr(subformulas1), LTLfOr(subformulas2)])
                l_args -= 2
            # return LTLfOr([args[0], tmp_l])
            subformulas1 = [LTLfNot(args[0]), tmp_l]
            subformulas2 = [args[0], LTLfNot(tmp_l)]
            # subformulas = [LTLfOr(subformulas1), LTLfOr(subformulas2)]
            # return LTLfEquivalence(subformulas)
            return LTLfAnd([LTLfOr(subformulas1), LTLfOr(subformulas2)])
        else:
            raise ParsingError
    ...
    def ltlf_implication(self, args):
        ...
        if len(args) == 1:
            ...
            tmp_l = args[l_args-1]
            if(l_args>3):
                tmp_l =  LTLfOr([LTLfNot(args[l_args-3]),args[l_args-1]])
                l_args -= 2
            while(l_args >3):
                tmp_l = LTLfOr([LTLfNot(args[l_args-3]),tmp_l])
                l_args -= 2
            # return LTLfOr([args[0], tmp_l])
            # subformulas = [LTLfNot(args[0]),tmp_l]
            return LTLfOr([LTLfNot(args[0]),tmp_l])
        else:
            raise ParsingError
    ...
    def ltlf_or(self, args):
        ...
        if len(args) == 1:
            ...
            tmp_l = args[l_args-1]
            if(l_args>3):
                tmp_l =  LTLfOr([args[l_args-3], args[l_args-1]])
                l_args -= 2
            while(l_args >3):
                tmp_l = LTLfOr([args[l_args-3], tmp_l])
                l_args -= 2
            return LTLfOr([args[0], tmp_l])
        else:
            raise ParsingError
    ...
    def ltlf_and(self, args):
        ...
        if len(args) == 1:
            ...
            tmp_l = args[l_args-1]
            if(l_args>3):
                tmp_l =  LTLfAnd([args[l_args-3], args[l_args-1]])
                l_args -= 2
            while(l_args >3):
                tmp_l = LTLfAnd([args[l_args-3], tmp_l])
                l_args -= 2
            return LTLfAnd([args[0], tmp_l])
            subformulas = args[::2]
    ...
    def ltlf_until(self, args):
        ...
        if len(args) == 1:
            ...
            tmp_l = args[l_args-1]
            if(l_args>3):
                tmp_l =  LTLfUntil([args[l_args-3], args[l_args-1]])
                l_args -= 2
            while(l_args >3):
                tmp_l = LTLfUntil([args[l_args-3], tmp_l])
                l_args -= 2
            return LTLfUntil([args[0], tmp_l])
        else:
            raise ParsingError
    # release 转成等价的其他运算符的公式，实际的模型训练并没有使用release
    def ltlf_release(self, args):
        """Parse LTLf Release."""
        if len(args) == 1:
            return args[0]
        elif (len(args) - 1) % 2 == 0:
            return LTLfNot(LTLfUntil([LTLfNot(args[0]), LTLfNot(args[2])]))
        else:
            raise ParsingError
